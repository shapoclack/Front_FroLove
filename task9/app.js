...
let users = [];
let products = [];
const refreshTokens = new Set(); // Множество активных refresh-токенов
...
const swaggerOptions = {
    definition: {
        openapi: '3.0.0',
        info: {
            title: 'API AUTH WITH JWT + Refresh Tokens',
            version: '1.0.0',
            description: 'API для изучения JWT авторизации с refresh-токенами (Задание №9)',
        },
        servers: [
            {
 ...
 * /api/auth/login:
 *   post:
 *     summary: Авторизация пользователя (выдача access + refresh токенов)
 *     tags: [Auth]
 *     requestBody:
 *       required: true
 *       content:
...
app.post("/api/auth/login", async (req, res) => {
    ...
    // Сохраняем refresh-токен в множество активных токенов
 ...
 *             properties:
 *               refreshToken: { type: string, description: Текущий refresh-токен }
 *     responses:
 *       200:
...
app.post("/api/auth/refresh", (req, res) => {
    ...
    // Проверяем, что токен есть в списке разрешённых
    ...
    jwt.verify(refreshToken, REFRESH_SECRET, (err, decoded) => {
        if (err) {
            // Если токен невалиден — удаляем из списка
        ...
        // Добавляем новый refresh-токен в список
